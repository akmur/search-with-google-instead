...
var SearchWithGoogleInstead = (function(){
	...
	var getEngineType = function(url){
	...
	var insertButtonHTML = function(buttonHTML){
		searchField.closest('div').style.position = 'relative';
		searchField.closest('div').insertAdjacentHTML('beforeend', buttonHTML);
	}

	var insertStyles = function(engineName) {
		var styles = engineName === 'duckduckgo' ? styleDuckDuckGo : styleBing;
	...
	publicAPI.init = function(){
		var engineName = getEngineType(window.location.href);
		var buttonHTML = buildButtonHTML();
